...
Clear-Host
(Get-Counter -ListSet 'Disque physique').Counter

#The Paths property returns the same thing as the Counter property
Clear-Host
(Get-Counter -ListSet 'Disque physique').Paths
(Get-Counter -ListSet 'Disque physique').Counter

Clear-Host
Get-Counter -ListSet 'Disque physique'

Clear-Host
Compare-Object -ReferenceObject (Get-Counter -ListSet 'Disque physique').Counter -DifferenceObject (Get-Counter -ListSet 'Disque physique').Paths

#Counter is an alias property of Paths
Get-Counter -ListSet 'Disque physique' | Get-Member -MemberType Properties

#Trying to filter down to specific counters or paths does not work like you think it should
(Get-Counter -ListSet 'Disque physique' | Where-Object Counter -like '*file*').Counter
(Get-Counter -ListSet 'Disque physique' | Where-Object Paths -like '*file*').Counter

#That's because they're all returned as one item
(Get-Counter -ListSet 'Disque physique').Count
(Get-Counter -ListSet 'Disque physique').Counter.Count

#Select the Counter property first and then filter the data
(Get-Counter -ListSet 'Disque physique').Counter | Where-Object {$_ -like '*file*'}
((Get-Counter -ListSet 'Disque physique').Counter | Where-Object {$_ -like '*file*'}).Count
...
Clear-Host
Get-Counter -Counter '\Disque physique(*)\% d''inactivité' | Get-Member -MemberType Properties

<#
    There’s a Timestamp property which is self-explanatory.
#>

(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').Timestamp
...
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').CounterSamples
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').CounterSamples.Count
((Get-Counter -Counter '\Disque physique(*)\% d''inactivité').CounterSamples | Get-Member).TypeName[0]
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').CounterSamples | Get-Member
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').CounterSamples | Select-Object -Property * -First 1
...
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').Readings
(Get-Counter -Counter '\Disque physique(*)\% d''inactivité').Readings.Count
((Get-Counter -Counter '\Disque physique(*)\% d''inactivité').Readings | Get-Member).TypeName[0]
...
(Get-Counter -ListSet 'Disque logique').PathsWithInstances | Where-Object {$_ -like '*espace libre*'}
((Get-Counter -ListSet 'Disque logique').PathsWithInstances | Where-Object {$_ -like '*espace libre*'}) -replace '^.*\(|\).*$'
Get-Counter -Counter '\Disque Logique(C:)\% d''espace libre', '\Disque Logique(E:)\% d''espace libre'
Get-Counter -Counter '\Disque logique(*)\% d''espace libre' | Where-Object CounterSamples -NotLike '*_Total*'
(Get-Counter -Counter '\Disque logique(*)\% d''espace libre').CounterSamples
(Get-Counter -Counter '\Disque logique(*)\% d''espace libre').CounterSamples | Where-Object InstanceName -NotLike '_Total'

#Query all of the logical disk performance counters
Get-Counter -ListSet 'Disque logique'
(Get-Counter -ListSet 'Disque logique').Counter
(Get-Counter -ListSet 'Disque logique').Counter | Get-Counter
Get-Counter -ListSet 'Disque logique' | Get-Counter

#Store the counter(s) in a variable
$LogicalDisk = '\Disque logique(*)\% d''espace libre'
$LogicalDisk | Get-Counter -MaxSamples 5

#Store the counters in a variable and then query them
$LogicalDisk = (Get-Counter -ListSet 'Disque logique').Counter
Get-Counter -Counter $LogicalDisk

#Query all of them three times, every two seconds
Get-Counter -ListSet 'Disque logique' | Get-Counter -SampleInterval 2 -MaxSamples 3

#Query all of them continuously
Get-Counter -ListSet 'Disque logique' | Get-Counter -Continuous
...
Get-Counter -ListSet 'Disque logique' | Get-Counter -Continuous -MaxSamples 10

#The default and minimum value for SampleInterval is 1. The default and minimum value for MaxSamples is also 1.
Get-Counter -ListSet 'Disque logique' | Get-Counter -SampleInterval 0 -MaxSamples 10
Get-Counter -ListSet 'Disque logique' | Get-Counter -SampleInterval 1 -MaxSamples 10 -OutVariable Results
$Results
...
Get-Counter -ComputerName DC01 -Counter '\Disque logique(*)\% d''espace libre'
Get-Counter -Counter '\\DC01\Disque logique(*)\% d''espace libre'
...
$DiskSpace = Get-Counter -Counter '\Disque logique(*)\% d''espace libre' -ComputerName DC01
$DiskSpace
...
(Get-Counter -ComputerName DC01 -Counter '\Disque physique(*)\% d''inactivité').CounterSamples |
Select-Object -Property @{label='ComputerName';expression={
$_.Path -replace '^\\\\|\\.*$'}},
@{label='Object';expression={
$_.Path -replace "^\\\\$env:COMPUTERNAME\\|\(.*$"}},
...
Describe 'Current Disk Queue Length' {
    It 'Should not be higher than 2' {
        (Get-Counter -Counter '\Disque physique(* c:)\Taille de file d''attente du disque actuelle'
...
Get-Counter -Counter '\Disque physique(*)\% d''inactivité'
...
Get-Counter -Counter '\Mémoire\Octets disponibles'
...
Get-Counter -Counter '\Interface réseau(*)\Total des octets/s'
...
Invoke-Pester -Script .\Performance-Counter-Validation.Tests.french.ps1
...
Get-Counter -Counter '\Processeur(*)\% temps processeur' | Export-Counter -Path $Path\Counters.blg -Force
...
Get-Counter -Counter '\Processeur(*)\% temps processeur' -MaxSamples 2 | Export-Counter -Path $Path\Counters.blg -Force
Import-Counter -Path $Path\Counters.blg

#Convert the format of an existing exported performance log
Get-Counter -Counter '\Processeur(*)\% temps processeur' -MaxSamples 10 | Export-Counter -Path $Path\Counters.blg -Force
...
$Counters = '\Mémoire\Octets disponibles',
            '\Mémoire\Pages/s',
            "\Disque physique(*)\% d’inactivité",
            '\Disque physique(*)\Moyenne disque s/lecture',
            '\Disque physique(*)\Moyenne disque s/écriture',
...
Import-Counter -Path C:\Demo\Results.blg -Counter '\Mémoire\Octets disponibles'
